...
{
    bool imking = false, musicon = false, sonando = false;
    ...
    public GameObject IMG_Perder, wait;
    ...
    void Start()
    {
        soundtrack = this.gameObject.GetComponent<AudioSource>();
        Time.timeScale = 0;
        OnlineInitializer.OnlinePlayers = OnlineInitializer.OnlinePlayers+1;

        if(OnlineInitializer.OnlinePlayers <= 2){
            imking = true;
        }

        Debug.Log("" + OnlineInitializer.OnlinePlayers);
        localWinner =false;
        if(isLocalPlayer)
        {
    ...
    void Update()
    {
        ...
        if(OnlineInitializer.OnlinePlayers > 1 && imking == true)
        {
